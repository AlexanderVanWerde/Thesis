...
This may be stated in more generally terms of $\D_X\TMinpace\left\langle s_1,\ldots,s_n, t \right\rangle$-modules.
Here $t$ is a new variable which commutes with sections of $\D_X$ and satisfies $ts_i - s_it = 1$ for any $i=1,\ldots, n$.
For a $\D_X\TMinpace\left\langle s_1,\ldots,s_n, t \right\rangle$-module $\M$ one defines $B_\M$ as the ideal of global section of $\Ann_{\underline{\C[s]}} \M/t\M$.
Viewing $\D_X[s]F^s$ as a $\D_X\TMinpace\left\langle s_1,\ldots,s_n, t \right\rangle$-module using the action $tF^s = F^{s + 1}$ this agrees with the earlier definition.
...
Based on appendix \cite{budur2019zero} which is itself based on \cite{bjork1979rings}.
\begin{definition}
...
\end{lemma}
\begin{lemma}
  Let $\M$ be a relative holonomic $\A_X^R$-module which is $(n+k)$-pure for some $0\leq k \leq \dim R$. If $b\in R$ is not contained in any minimal prime ideal containing $B_\M$ then multiplication by $b$ induces injective automorphisms on $\N$ and $\Ext^{n+l}_{\A_X^R}(\M,\A_X^R)$.
\end{lemma}
...
\end{definition}
\begin{lemma}
  Relative holonomic is CM over a open $\Spec R$.
\end{lemma}
\begin{proof}
  Like lemma 3.5.2
\end{proof}
...
This section employs the same notation as \cref{sec: IntoductionChapterRelative}. Recall that the global Bernstein-Sato Ideal is the intersection of all local ones. This means that the global Bernstein-Sato zero locust $Z(B_F)$ is the union of all local ones so it suffices to estimate $Z(B_{F,x})$.
...
in $\A_X F^s \otimes_{\O_X}\Omega_X$. This viewpoint is convenient since the $\A_X$-module direct image is more natural for right modules.
...
This may be viewed as a $\Yt$-module by the action $t G^s \mu^*(dx) = G^{s+1} \mu^*(dx)$. Similarly to
\begin{lemma}\label{lem: BernsteinSatoPolynomialUpstairs}
  ...
  $\prod_i \prod_j(\operatorname{mult}_{E_i}(g_1) s_1 + \cdots + \operatorname{mult}_{E_i}(g_r)s_r + k_i + c_j)$
  with $c_j\in \Z_{\geq 0}$ belongs to the Bernstein-Sato ideal $B_\M$.
\end{lemma}
\begin{proof}
  It suffices to take an open cover $\{U_\alpha \}$ of $Y$ such that this polynomial is a section of $\Ann_{\underline{\C[s]}} \M/t\M$ over $ U_\alpha \times \C^p)$ for any open $U_\alpha$ of the cover.
  Take $U$ sufficiently small so that it admits local coordinates $z_1,\ldots,z_n$ where $z_i$ determines $E_i$ if $E_i\cap U \neq \emptyset$.
...
\end{proof}
By \cref{lem: GlobalSectionDirectImage} the global section $G^s \mu^*(dx)$ of $\M$ gives rise to a global section $u$ of $\int^0\M$. Denote $\U$ for the right $\A_X^R$-module generated by $u$.
...
\end{lemma}
\begin{proof}
   The resolution of singularities $Y\to X$ is a isomorphism on the complement of $\prod f_i = 0$. Hence, a isomorphism $\U = \int^0 \M \cong  \A_X^RF^s  \otimes_{\O_X}\Omega_X$ holds outside of $\prod f_i = 0$.
...
\end{proof}
Due to  \cref{lem: BernsteinSatoPolynomialUpstairs} there is a $b$-polynomial of a desirable form in $B_{\int^0 \M}$. The remaining difficulty in the proof is to go from there to a $b$-polynomial in $B_\U$. Note that $\int^0\M$ is certainly a relative holonomic $\A_X^R$-module by Kashiwara's estimate \cref{thm: KashiwaraEstimate}.
\begin{lemma}
  Let $\N$ be a relative holonomic $\A_X^R$-module with the structure of a $\Xt$-module. If there exists some $B(s_1,\ldots,s_n)\in \C[s_1,\ldots,s_n]$ such that $B\N = 0$ then $t^N \N = 0$ for sufficiently large $N$.
\end{lemma}
Note: Check how necessary open set for CM matters 
\newpage
\section{Old Stuff}
\subsection{Sketch}
\begin{enumerate}
  ...
  \item By Kashiwara's estimate $\int^0 \M$ is relative holonomic and this contains a global section $u$.
  \item There is a surjection $u\to f^s$.
  \item Goal: Given $b$ which annihilates $\M/t\M$ show $B(s) = \prod b(s + k)$ annihilates $u/tu$.
  \begin{itemize}
    \item Problem: the usual argument exploits that $\int^0 \M /u$ is finite length.
  ...
  \begin{itemize}
    \item $b \int^0 \M \subseteq t\int^0\M$
    \item $t^N\int^0\M / u = 0$ for large $N$
    \item Hence $Bu \subseteq B\int^0\M \subseteq t^{N+1}\int^0 \M \subseteq tu$
  \end{itemize}
  In the final step of the induction we get that $(\int^0\M \otimes \frac{\C[s]}{L}) / \widetilde{u}$ is holonomic from which we can deduce $t^N \int^0 \M \otimes \frac{\C[s]}{L} / \widetilde{u} = 0$. But we actually need $t^N (\int^0\M/ u) \otimes \frac{\C[s]}{L} = 0$
  ...
  \end{itemize}
  \item Now
  $$0 \to u \to \int^0 \M \to \frac{\int^0 \M}{u} \to 0 $$
  induces
  $$\begin{tikzcd}
    Tor\arrow{r} & u\otimes \frac{\C[s]}{\ell}\arrow{r}\arrow{rd} & (\int^0 \M) \otimes \frac{\C[s]}{\ell}\arrow{r}\arrow[hookrightarrow]{d} & \frac{\int^0 \M}{u} \otimes \frac{\C[s]}{\ell}\arrow{r} & 0\\
