...
\end{definition}
The following obvious isomorphisms of filtered modules allow us to view the relative Rees module as a parametrisation of gradings with different steps.
$$\frac{\Rrel\M}{(z-1)\Rrel\M} \cong \M; \qquad \frac{\Rrel\M}{z\Rrel\M} = \grrel \M;\qquad \frac{\Rrel\M}{z^\ell\Rrel\M} =  \grl\M.$$
Here $\gr_{[\ell]}^{rel}$ takes a filtered object and returns the graded object $\oplus_k F_k / F_{k-\ell}$.
In particular $\grl\M$ is a graded $\gr_{[\ell]}\A_X^Y$-module.
The first step is to understand the interaction between direct images and $\gr_{[\ell]}^{rel} $ for large $\ell$.
The first formula may be also be used to find the corresponding filtered $\A_Y^R$-module for any graded $\Rees\A_Y^R$-module without $\C[z]$-torsion.
...
From now on we equip $\int^j \M$ with the good filtation inherited from the Rees module's direct image. By the formula relating Rees modules and $\gr_{[\ell]}^{rel}$ the direct image of Rees modules induces a direct image of $\gr_{[\ell]}\A_Y^R$-modules.
\begin{lemma}
    If $\ell$ is sufficiently large then $\grl \int^j\M$ is a subquotient of $\int^j\grl\M$.
\end{lemma}
\begin{proof}
...
\begin{definition}
  For any filtered $\A_Y^R$-module $\M$ and $\ell \geq 1$ the $G$-filtration on $\grl \M$ is defined by
  $$G_j \M = \bigoplus_k \frac{F_{k+j - \ell}\M}{F_{k-\ell}\M}. $$
\end{definition}
\begin{lemma}
  For any filtered $\A_Y^R$-module $\M$ and $\ell \geq 1$ one has the following isomorphism of $\gr \A_Y^R$-modules
  $$\gr^G \grl \M \cong (\grrel\M)^\ell.$$
\end{lemma}
\begin{proof}
  This is immediate by a direct computation. 
\end{proof}
