...
For notational simplicity the proof will be done in the algebraic case and we discuss the neccesary modifications for the local analytic case at the end.
...
\section{Direct Image Functor for $\A_X^R$-modules}
  ...
  The analytic analogue of $\A_{Y}^R$ is given by $\D_{Y\times U/U}:= \D_Y\otimes_{\O_Y}\O_{Y\times U}$ and it is clear how notions such as the relative characteristic variety may be analytified.
...
\begin{proof}
  ...
  Recall from \cref{lem: ExtGrade} terms with $p>n$ have grade greater than $n$ and due to \cref{prop: AXRBehaviourGradesJAndChrel} there are no non-zero terms with $p<n$.
...
\begin{lemma}\label{lem: StabilisationtN}
  Let $\M$ be a relative holonomic $\A_X^R$-module which comes equipped with the structure of a $\Xt$-module and let $x\in X$.
  Suppose that $\M$ has grade $j(\M) = n+k$ with $k\geq 1$.
  Then there exists a open $\Spec R' \subseteq \Spec R$ such that $\M' = \M \otimes_{R} R'$ is a relative holonomic $\A_X^{R'}$-module with $t^N \M_x' = 0$ for $N$ sufficiently large. Moreover, it may be assumed that $\Spec R \setminus \Spec R'$ has codimension strictly greater than $k$.
\end{lemma}
\begin{proof}
  The proof is split in two main parts. The first part establishes that $\Ex{\A_X^{R'}}{n+k}(t^i\M',\ab\A_X^{R'})$ stabilises and that the $t^i\M'$ are $(n+k)$-Cohen-Macauley. It follows that $t^i\M'$ stabilises and the final part of the proof deduces that the stable value is zero.
  ...
  By the Cohen-Macauley results which have been established it follows that the morphisms $\E^{n+k}( t^i\M') \to  \E^{n+k}(t^{i+1}\M')$ and $\E^{n+k}(\M') \to \E^{n+k} (t^i\M')$ are surjective.
