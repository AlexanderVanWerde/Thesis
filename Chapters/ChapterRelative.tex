...
\begin{proposition}\label{prop: ProjectiveResolutionLength}
  Any coherent $\D_X^R$-module admits a projective resolution of length $\leq 2n + r$.
\end{proposition}
\begin{proof}
  .\todo{By truncation}
\end{proof}
...
\begin{proposition}
  ...
  converging to $H^{p+q} R\Hom_{\D_X^R}^p(\M,\D_X^R)\otimes_{\D_X^R}^L \N.$
\end{proposition}
\begin{proof}
  ...
  Observe that $\Hom_{\D_X^R}(\F^j, \D_X^R)$ is still a locally free module due to $\F^j$ having finite rank.
  In particular these are projective so they are acyclic for $\blank\otimes_{\D_X^R}^L \N$.

  It is a standard result due to Cartan-Eilenberg that there exists a double complex $\mathscr{P}^{\bullet\bullet}\to \Hom_{\D_X^R}(\F^{\bullet},\D_X^R)$ of projectives $\mathscr{P}^{pq}$ such that
  \begin{enumerate}
    \item[(i)] The maps $\mathscr{P}^{\bullet q}\to \Hom_{\D_X^R}(\F^q,\D_X^R)$ yield projective resolutions.
    \item[(ii)] The images and kernels of the horizontal differentials are projective modules.
    \item[(iii)] The horizontal cohomology $H_h^{q}(\mathscr{P}^{\bullet\bullet})$ is a complex and yields a projective resolution of $\Ex{\D_X^R}{q}(\M, \D_X^R)$.
  \end{enumerate}
  Moreover, using \cref{prop: ProjectiveResolutionLength} in the construction of the Cartan-Eilenberg resolution allows one to assume that $\mathscr{P}^{pq}=0$ for $p>2n + q$.

  Consider the double complex $\mathscr{P}^{\bullet \bullet}\otimes_{\D_X^R}\N$.
  There are two spectral sequences associated to this double complex.
  ...
  \end{align*}
  where it was used that the image and cohomologies of the horizontal differentials are projective.

  Since the double complex is vertically bounded it follows from \cref{prop: FiltrationSpectral} that both spectral sequences converge to the same total cohomology.
  Since the first spectral sequence collapses on the second sheat this means that $\Tr{\D_X^R}{-p}(\Ex{\D_X^R}{q}(\M,\D_X^R), \N)$ converges to $H^{p+q} R\Hom_{\D_X^R}(\M,\D_X^R)\otimes_{\D_X^R}^L \N$
\end{proof}
